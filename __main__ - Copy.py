...
class StatusBarProgressHelper(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        super(StatusBarProgressHelper, self).__init__(*args, **kwargs)
...
class CCDWindow(QtGui.QMainWindow):
    ...
    def initUI(self):
        ...
        self.hwpSweepProgress = StatusBarProgressHelper(self)
        ...
        actionToWidgets = [(self.ui.fExpTypeAbs, AbsWid),
                           ...
                           (self.ui.fExpTypeHSG_TimeDomain, HSGTimeWid)
                           ]
        ...
        self.ui.cSpecGPIB.setAddress('Fake') # Regensburg doesn't have it
        ...
        self.thzPowSweep = self.ui.mFileSweepTHz
        self.thzPowSweep.triggered.connect(self.startTHzPowSweep)
        self.thzPowSweep.setCheckable(True)

        self.ui.mFileTimeDelaySweep.triggered.connect(self.startTimeDelaySweep)
        self.ui.mFileTimeDelayPolarimetry.triggered.connect(self.startTDPolarimetrySweep)
    ...
    def updateExperiment(self, b):
        ...
        self.CCD.doEMGain = self.expUIs[sent].doEMGain
    ...
    def addDelayStage(self):
        try:
            from InstsAndQt.PIDelayStage.C863Panel import C863Panel
        except ImportError:
            MessageDialog(self, "Error importing module for PI Delay Stage")
            return

        motorDriverGB = QtGui.QGroupBox("Delay Stage", self)
        motorDriverGB.setFlat(True)
        layout = QtGui.QVBoxLayout()
        self.delayStage = C863Panel()
        layout.addWidget(self.delayStage)
        motorDriverGB.setLayout(layout)
        self.ui.miscToolsLayout.addWidget(motorDriverGB)
    ...
    def startTHzPowSweep(self, val):
        ...
        self.thDoSweep.args = None
        self.thDoSweep.target = self.doTHzPowerSweep
    ...
    def startHWPSweep(self, val):
        if val:
            ...
            start, ok = QtGui.QInputDialog.getDouble(self, "Starting val", "Start", 0)
            if not ok:
                self.ui.mFileDoHWPSweep.setChecked(False)
                return

            stop, ok = QtGui.QInputDialog.getDouble(self, "Stopping val", "Stop", 360)
            if not ok:
                self.ui.mFileDoHWPSweep.setChecked(False)
                return

            step, ok = QtGui.QInputDialog.getDouble(self, "Stepping val", "Step", 360/16)
            if not ok:
                self.ui.mFileDoHWPSweep.setChecked(False)
                return

            numIm, ok = QtGui.QInputDialog.getInt(self, "Number of images", "Number of images", 4)
            if not ok:
                self.ui.mFileDoHWPSweep.setChecked(False)
                return
            ...
            self.thDoSweep.target = self.hwpSweepLoop
            self.thDoSweep.start()
        else:
            pass
    ...
    def hwpSweepLoop(self, args=(0, 365, 20, 4, None)):
        ...
        if len(args)==4:
            start, stop, step, numImages, callback = *args, None
        else:
            start, stop, step, numImages, callback = args
        if callback is None:
        ...
        for hwpAngle in points:
            ...
            self.getCurExp().processImageSequence()
    ...
    def startTimeDelaySweep(self, val):
        if not val: return # unchecked it
        if not isinstance(self.getCurExp(), HSGTimeWid):
            log.warning("The time delay widget is required for TD scans")
            MessageDialog(self, "The Time Delay experimental widget must be selected")
            self.ui.mFileTimeDelaySweep.setChecked(False)
            return

        log.debug("Starting time delay sweep")
        ...
        try:
            start = float(self.getCurExp().ui.tTDStart.text())
            step = float(self.getCurExp().ui.tTDStep.text())
            end = float(self.getCurExp().ui.tTDEnd.text())
    ...
    def doTimeDelaySweep(self, args=(0, 50, 1000, 4, None)):
        if len(args)==4:
            start, step, end, nimg, callback = *args, None
        else:
            start, step, end, nimg, callback = args
        if callback is None:
            callback = lambda: None
    ...
    def startTDPolarimetrySweep(self, val):
        if not val: return # unchecked it
        log.debug("Starting TD polarimetry sweep")
        ...
        try:  # Check/get parameters for the TD portion of the sweep
        ...
        polstop, ok = QtGui.QInputDialog.getDouble(self, "Stopping val", "Stop", 360)
        if not ok:
            self.ui.mFileTimeDelayPolarimetry.setChecked(False)
            return

        polstep, ok = QtGui.QInputDialog.getDouble(self, "Stepping val", "Step", 360/16)
        if not ok:
            self.ui.mFileTimeDelayPolarimetry.setChecked(False)
            return
    ...
    def doTDPolarimetrySweep(self, args = (0, 50, 1000, 0, 260, 22.5, 4)):
        tdstart, tdstep, tdend, polstart, polstop, polstep, numIm = args
    ...
    def startSweepLoop(self, val):
        if hasattr(val, '__iter__'):
            ...
            self.thDoSweep.target = self.sweepLoop
            self.thDoSweep.start()
    ...
    def startConsecutiveImages(self, val):
        ...
        self.thDoSweep.args = doBG
        self.thDoSweep.target = self.doConsecutiveLoop
        self.thDoSweep.start()

    def doConsecutiveLoop(self, doBG = False, checkChecked = None, callback = None,
                          procSequence = False):
        """

        :param doBG:
        :param checkChecked: what ui element do you check to see if it's checked
        to detemrine if you want to exit the loop or not?
        :param callback: a function to call between each loop
        :param procSequence: bool for whether or not the image sequence should be processed
        :return:
        """
        numImages = 0
        # print("Starting Consecutive", self.consec.isChecked())
        ...
        if checkChecked is None:
            checkChecked = self.consec

        while numImages < self.consecImages:
            if not checkChecked.isChecked(): break
            log.debug("Consecutive image: {}".format(numImages))
            # self.getCurExp().ui.bCCDImage.clicked.emit(True)
            self.getCurExp().takeImage(isBackground=doBG)  ### Should really be called 'prepareAcq' or something
            # time.sleep(0.25)
            if doBG:
                self.getCurExp().doExposure(self.getCurExp().processBackground)
            else:
                self.getCurExp().doExposure(self.getCurExp().processImage)
            # self.getCurExp().thDoExposure.wait()
            # numImages += 1
            try:
                if doBG:
                    numImages = self.curExp.prevBackEMCCD.imageSequence.numImages()
                else:
                    numImages = self.curExp.prevDataEMCCD.imageSequence.numImages()
            except:
                # If, for some reason, prevData is undefined. Dunno why.
                log.exception("Couldn't find the iamge sequence?")
                numImages = 0
        ...
        if procSequence:
            if doBG:
                ...
                log.debug("Automatically processing image sequence")
                self.getCurExp().processImageSequence()
