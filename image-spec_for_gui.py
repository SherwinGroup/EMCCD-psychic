...
#            return self.hsg_data[args[0]]
    ...
    def integrate(self):
        ...
        '''
        self.spectrum = self.clean_array[:,self.y_min:self.y_max].sum(axis=1)        
...
class HSG_image(EMCCDimage):
    ...
    '''
    def __init__(self, image_array, bg_array, description, hsg_dict, experiment_dict):
        '''
        ...
        experiment_dict = the important nonspecific conditions:
            CCD_temperature
            exposure
            gain
            y_min
...
#        self.sample = None # Need to add this
...
#        self.dark_region = None # Currently the only idea I have for this is to set it manually during acquisition
#                                # I imagine it to be a tuple or something that sets start and end columns
#        
...
#            self.save_clean_bg()
#        
#        self.raw_array = np.genfromtxt(fname, comments='#', delimiter=',')
#         
#        
...
#        self.subtrahenda = []
    ...
    def save_spectrum(self, name_str, sweep=None, iternum=''):
        ...
        work yet.  I want it to be able to iterate and not overwrite files.  It
        should also output the data in an Origin-friendly manner.  I'm pretty 
        sure it will automatically be matplotlib-friendly.
        ...
        if sweep is None:
            fname = '{0}{1}.txt'.format(name_str, iternum)
        elif sweep == 'NIR_power':
            fname = '{0} {1}mW{2}.txt'.format(name_str, self.NIR_power, iternum)
            my_header.append('\n,' + self.NIR_power)
        elif sweep == 'NIR_lambda':
            fname = '{0} {1}nm{2}.txt'.format(name_str, self.NIR_lambda, iternum)
            my_header.append('\n,' + self.NIR_lambda)
        elif sweep == 'THz_power':
            fname = '{0} {1}kW{2}.txt'.format(name_str, self.THz_power, iternum)
            my_header.append('\n,' + self.THz_power)
        elif sweep == 'THz_freq':
            fname = '{0} {1}GHz{2}.txt'.format(name_str, self.THz_freq, iternum)
            my_header.append('\n,' + self.THz_freq)
        else:
            print 'You fucked up!'
            return None
        
        np.savetxt(fname, self.spectrum, delimiter=',', header=my_header, comments='')

    def save_images(self):
        '''
        Saves the raw image_array, the blank array, and the bg array
        '''
        raise NotImplementedError

class PL_image(EMCCDimage):
    '''
    This class is for handling PL images and turning them into simple spectra.
    '''
    
    def __init__(self, image_array, bg_array, pl_dict, exp_dict):
        '''
    ...
    def save_spectrum(self, name_str, iternum=''):
        '''
        This will save the 2x1600 PL spectrum
        '''
        raise NotImplementedError
...
class Abs_image(EMCCDimage):
    ...
    def __init__(self, image_array, blank_array, bg_array, abs_dict, experimental_dict):
        '''
    ...
    def calculate_absorbance(self):
        '''
        This should divide out the relevant blank spectrum.
        '''
        raise NotImplementedError
